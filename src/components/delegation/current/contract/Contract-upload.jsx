...
import {
  GET_FILE_URL,
  SELECT_DELEGATION_CONTRACT_MANAGER_FAIL_TEXT,
  UPLOAD_PDF_FILE,
  SAVE_DELEGATION_ENTERPRISE_CONTRACT_URL,
  SELECT_DELEGATION_CONTRACT_URL,
  DOWNLOAD_DELEGATION_CONTRACT_WORD,
} from '@/constants/api-constants';

export default Form.create({ name: 'contractEnterprise' })(({ form }) => {
  const { getFieldDecorator, setFieldsValue, getFieldValue } = form,
    { enterpriseDelegationUuid, delegationSteps } = useSelector(
    ...
    form.validateFields(async (err, value) => {
      if (enterpriseDelegationUuid) {
        if (!err) {
          ...
          await proxyFetch(SAVE_DELEGATION_ENTERPRISE_CONTRACT_URL, value);
  ...
  useEffect(() => {
    if (enterpriseDelegationUuid && needContractStatus) {
      dispatch(enterpriseAction.asyncSetDelegationSteps(enterpriseDelegationUuid));
    }
  }, [enterpriseDelegationUuid, needContractStatus, dispatch]);
  ...
  const handleUploadFile = async (file) => {
    if (handleBeforeUpload(file)) {
      ...
      const fileUrl = await proxyFileFetch(UPLOAD_PDF_FILE, {
        file: file.file,
        folderName: 'delegation/enterpriseContract',
      });
  ...
  return (
    <>
      ...
      {managerFailText && delegationSteps[1].status === -1 ? (
        <div className='electronic-contract-alert-box'>
          <Alert
            message='填写错误,请按描述修改'
