...
// 样式
import '@/style/home/delegation/product.styl';
  ...
  useEffect(() => {
    if (enterpriseDelegationUuid) {
      (async () => {
        ...
        // 数据回显
        if (delegationProduct && delegationProduct.url) {
          // 数据处理
          setFieldsValue({ productUrl: [delegationProduct.url] });
          setIsNeedUrlFresh(true);
        }

        if (delegationProduct.failText) {
          setFailText(delegationProduct.failText);
        }

        delete delegationProduct.status;
        delete delegationProduct.statusText;
        delete delegationProduct.failText;
    ...
    form.validateFields(async (err, value) => {
      if (enterpriseDelegationUuid) {
        if (!err) {
          ...
          const res = await proxyFetch(SAVE_DELEGATION_PRODUCT, value);
